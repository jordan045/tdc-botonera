...

{
    ...
    auto *botonMensaje = new QPushButton("",this);
    auto *logicaMensaje = new Boton(this,"mensaje");

    range_group->addButton(botonMensaje,8);

    layout->addWidget(botonMensaje);
    gui_buttons.append(botonMensaje);

    QString style = QString("QPushButton {image: url(':/range/img/Range/%1.png')}"
                            "QPushButton:checked {image: url(':/range/img/Range/%1_pressed.png')}").arg(labels[1]);

    botonMensaje->setStyleSheet(style);

    QObject::connect(botonMensaje, &QPushButton::pressed, logicaMensaje, &Boton::sendMessage);
